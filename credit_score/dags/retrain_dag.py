...
Seance 17 - TP Airflow
    Pipeline simple : preparation des donnees -> entrainement -> controle qualite.
"""
...
def task_prepare_data(**context) -> None:
    # S17-1 : verifier que le dataset est present dans data/
    from config import DATA_PATH

    if not DATA_PATH.exists():
        raise FileNotFoundError(f"Dataset introuvable : {DATA_PATH}")
    logger.info("Dataset OK : %s (%d Ko)", DATA_PATH, DATA_PATH.stat().st_size // 1024)


def task_train(**context) -> None:
    # S17-2 : entrainer le modele et pousser f1 dans XCom
    from train import train

    metrics = train()
    context["ti"].xcom_push(key="f1", value=metrics["f1"])
    logger.info("Entrainement termine — f1=%.4f  roc_auc=%.4f", metrics["f1"], metrics["roc_auc"])


def task_check_quality(**context) -> None:
    # S17-3 : recuperer f1 depuis XCom et valider le seuil
...
with DAG(
    ...
    schedule="0 3 * * 1",  # S17-4 : tous les lundis a 3h du matin
